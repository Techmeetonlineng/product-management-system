...
async function register(req, res) {
  try {
    ...
    // PostgreSQL unique-constraint violation on users.email. This is a
    // defense-in-depth safety net: the findByEmail() pre-check above
    // already covers the common case, but two concurrent registrations
    // for the same address could both pass that check before either
    // INSERT completes, so the database's UNIQUE constraint is the
    // final authority.
    if (error.code === "23505") {
      return res.status(400).json({
        success: false,
